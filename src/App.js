...
export default function App() {
  ...
  async function loadChart() {
    const { data } = await supabase.from("chart").select("*").order("year", { ascending: false }).order("month", { ascending: false }).order("day", { ascending: true });
    if (!data) return;
    const grouped = {};
    data.forEach(row => {
    ...
    <div style={{background:"#080808",minHeight:"100vh",color:"#e8dfc0",fontFamily:"'Libre Baskerville',serif"}}>
      ...
      {loading ? (
        ...
        <div style={{padding:"16px 8px"}}>
          {sortedKeys.map(key => {
            const { year, month, days } = chartData[key];
            const daysInMon = new Date(year, month, 0).getDate();
            return (
              <div key={key} style={{marginBottom:24}}>
                {/* Month Header */}
                <div style={{background:"linear-gradient(135deg,#f0d080,#c9a84c,#8a6820)",padding:"10px",textAlign:"center",borderRadius:"4px 4px 0 0"}}>
                  <span className="pf" style={{color:"#000",fontWeight:900,fontSize:"1rem",letterSpacing:"0.15em"}}>
                    {MONTHS[month-1]} {year} — Dubai King
                  </span>
                </div>
                <table>
                  <thead>
                  ...
                  <tbody>
                    {Array.from({length: daysInMon}, (_, i) => i + 1).map(day => (
                      <tr key={day} style={{background: day%2===0?"#0d0d0d":"#080808"}}>
                        ...
                        <td className="pf" style={{color: days[day] && days[day]!=="XX" ? "#c9a84c" : "#444", fontWeight:700, fontSize:"1.1rem"}}>
                          {days[day] || "XX"}
                        </td>
                      </tr>
                    ))}
